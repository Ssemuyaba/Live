...
def filter_hls_playlist(hls_url):
    ...
    try:
        ...
        logger.error(f"HLS filter error: {e}")
        return hls_url
...
# -----------------------------
# 2️⃣ Get live matches (with caching + filter persistent match)
# -----------------------------
...
def live_matches():
    ...
    # Filter persistent match
    persistent_match_id = "estudiantes-de-rio-cuarto-san-lorenzo-15270128"
    filtered_matches = [m for m in normalized if m["match_id"] != persistent_match_id]
...
def match_detail(match_id):
    ...
    if data.get("success") and isinstance(data.get("data"), dict):
        detail = data["data"]
        streams.extend(collect_streams(detail))
